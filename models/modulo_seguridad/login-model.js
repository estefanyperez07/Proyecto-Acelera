...
LoginModel.searchToken = (token, cb) =>
  conn.query(
    "SELECT status, token FROM seguridad.tbl_ms_historic_token WHERE token=$1",
    [token],
    cb
  );
...
LoginModel.getByNameUser = (nameUser, cb) => {
  conn.query(
    "SELECT id_usuario,nombre_usuario,estado_usuario,id_rol,correo_electronico FROM seguridad.tbl_ms_usuario WHERE usuario = $1 and nombre_usuario != 'SYSTEMUSER' ",
    [nameUser],
    cb
  );
};
...
LoginModel.validatecurrentpassword = (data, cb) => {
  const text = "SELECT seguridad.ft_validate_current_password($1,$2)";
  const values = [data.id_usuario, data.contrasena];
  conn.query(text, values, cb);
};
